...
/**
 * Editor icon override stored at paths like `audienceItemIcons.0`.
 * All fields are optional — an object with only `bg: false` is valid and
 * removes the container background without changing the icon.
 *
 * These fields are EDITOR-ONLY and are never generated by AI:
 *   v    — named icon or uploaded URL
 *   size — display size in px
 *   mime — MIME type for color-mask decision (SVG/PNG vs JPEG)
 *   bg   — false = transparent container background
 */
export interface IconOverride {
  /** Named icon from the curated set, OR https:// URL for an upload.
   *  Absent = use the component's fallback element. */
  v?: string;
  /** Rendered size in px. Falls back to the component's `defaultSize`. */
  size?: number;
  /** MIME type stored at upload time for CSS-mask vs as-is rendering. */
  mime?: string;
  /** `false` = remove the container background/border. Default = show background. */
  bg?: boolean;
}
