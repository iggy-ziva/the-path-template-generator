(function () {
  ...
  var css = [
    ...
    '#fn-pill{display:inline-flex;align-items:center;gap:9px;padding:11px 18px 11px 14px;background:#0F0E0C;color:#F5F1EA;border:1px solid rgba(245,241,234,0.12);border-radius:9999px;font-size:13px;font-weight:600;letter-spacing:0.01em;cursor:pointer;box-shadow:0 4px 20px rgba(0,0,0,0.35);transform:translateY(80px);opacity:0;transition:box-shadow 200ms ease,border-color 200ms ease;will-change:transform,opacity;pointer-events:auto;}',
    '#fn-pill.fn-visible{animation:fn-bounce-in 600ms cubic-bezier(0.34,1.56,0.64,1) forwards;}',
    '#fn-pill:hover{box-shadow:0 6px 28px rgba(0,0,0,0.45);border-color:rgba(245,241,234,0.22);}',

    /* Pill dot */
    '#fn-dot{width:8px;height:8px;border-radius:50%;background:#D4A878;flex-shrink:0;position:relative;}',
    '#fn-dot::after{content:"";position:absolute;inset:-3px;border-radius:50%;border:1.5px solid rgba(212,168,120,0.5);animation:fn-pulse 2.4s ease-in-out infinite;}',

    /* Pill label */
    '#fn-pill-label{white-space:nowrap;}',

    /* Card — pointer-events re-enabled only when open */
    '#fn-card{background:#0F0E0C;border:1px solid rgba(245,241,234,0.1);border-radius:16px;overflow:hidden;box-shadow:0 16px 48px rgba(0,0,0,0.55),0 0 0 1px rgba(245,241,234,0.04);width:260px;transform-origin:bottom right;transform:scale(0.88) translateY(12px);opacity:0;pointer-events:none;transition:transform 320ms cubic-bezier(0.34,1.56,0.64,1),opacity 240ms ease;will-change:transform,opacity;}',
    '#fn-card.fn-open{transform:scale(1) translateY(0);opacity:1;pointer-events:auto;}',

    /* Card header */
    '#fn-card-header{display:flex;align-items:center;justify-content:space-between;padding:14px 16px 12px;border-bottom:1px solid rgba(245,241,234,0.08);}',
    '#fn-card-title{font-size:11px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:rgba(245,241,234,0.45);}',
    '#fn-close{width:24px;height:24px;border-radius:50%;background:rgba(245,241,234,0.06);border:none;color:rgba(245,241,234,0.5);display:grid;place-items:center;cursor:pointer;font-size:14px;line-height:1;transition:background 150ms ease,color 150ms ease;padding:0;}',
    '#fn-close:hover{background:rgba(245,241,234,0.12);color:#F5F1EA;}',
    ...
    '.fn-step:hover{background:rgba(245,241,234,0.05);transform:translateX(-3px);}',
    '.fn-step:active{transform:scale(0.97) translateX(-3px);}',
    '.fn-step.fn-active{border-left-color:#D4A878;background:rgba(212,168,120,0.06);}',
    '.fn-step.fn-active:hover{transform:translateX(-3px);}',

    /* Step number */
    '.fn-step-num{width:26px;height:26px;border-radius:50%;background:rgba(245,241,234,0.07);border:1px solid rgba(245,241,234,0.1);display:grid;place-items:center;font-size:11px;font-weight:700;color:rgba(245,241,234,0.4);flex-shrink:0;transition:background 150ms ease,color 150ms ease,border-color 150ms ease;}',
    '.fn-step.fn-active .fn-step-num{background:rgba(212,168,120,0.18);border-color:rgba(212,168,120,0.4);color:#D4A878;}',
    '.fn-step:hover:not(.fn-active) .fn-step-num{background:rgba(245,241,234,0.1);color:rgba(245,241,234,0.7);}',

    /* Step text */
    '.fn-step-text{display:flex;flex-direction:column;gap:1px;min-width:0;}',
    '.fn-step-name{font-size:13px;font-weight:600;color:rgba(245,241,234,0.65);white-space:nowrap;transition:color 150ms ease;}',
    '.fn-step.fn-active .fn-step-name{color:#F5F1EA;}',
    '.fn-step:hover:not(.fn-active) .fn-step-name{color:rgba(245,241,234,0.85);}',
    '.fn-step-sub{font-size:11px;color:rgba(245,241,234,0.3);white-space:nowrap;transition:color 150ms ease;}',
    '.fn-step.fn-active .fn-step-sub{color:rgba(212,168,120,0.65);}',

    /* Active arrow */
    '.fn-step-arrow{margin-left:auto;color:rgba(212,168,120,0.5);font-size:12px;opacity:0;transition:opacity 150ms ease,transform 150ms ease;flex-shrink:0;}',
    '.fn-step.fn-active .fn-step-arrow{opacity:1;}',
    '.fn-step:hover .fn-step-arrow{opacity:0.6;transform:translateX(2px);}',

    /* Divider between steps */
    '.fn-divider{height:1px;background:rgba(245,241,234,0.05);margin:2px 16px;}',
